...
int main() {
  ...
  for (int i = 0; i < n - 2; ++i) {
    for (int j = 0; j < n - 2; ++j) {
      string path = AStar(decided, pos[i * n + j + 1], Point(i, j));
      for (char k : path) {
        ans += move(k, pos[i * n + j + 1], pos[0]);
      }
      decided[i][j] = 1;
    }
    ...
    string path = AStar(decided, pos[i * n + n], Point(i, n - 2));
    for (char k : path) {
      ans += move(k, pos[i * n + n], pos[0]);
    }
    decided[i][n - 2] = 1;
  ...
  for (int j = 0; j < n - 2; ++j) {
    if (G[n - 1][j] == n * (n - 1) + j + 1 &&
        G[n - 2][j] == n * (n - 2) + j + 1) {
      decided[n - 1][j] = 1;
      decided[n - 2][j] = 1;
      continue;
    }
    string path = AStar(decided, pos[n * (n - 1) + j + 1], Point(n - 2, j));
    for (char k : path) {
      ans += move(k, pos[n * (n - 1) + j + 1], pos[0]);
    }
    decided[n - 2][j] = 1;
    if (G[n - 1][j] == 0 && G[n - 1][j + 1] == n * (n - 2) + j + 1) {
      ...
      decided[n - 2][j] = 1;
      decided[n - 1][j] = 1;
    } else if (G[n - 1][j] == n * (n - 2) + j + 1 && G[n - 2][j + 1] == 0) {
      ...
      path = AStar(decided, pos[n * (n - 2) + j + 1], Point(n - 2, j + 1));
      for (char k : path) {
        ans += move(k, pos[n * (n - 2) + j + 1], pos[0]);
      }
      decided[n - 2][j + 1] = 1;
      decided[n - 2][j] = 0;

      path = AStar(decided, pos[n * (n - 1) + j + 1], Point(n - 1, j));
