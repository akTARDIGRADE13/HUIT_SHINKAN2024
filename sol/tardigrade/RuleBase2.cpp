...
int main() {
    ...
    string ans = "";
    for(int i = 0; i < n - 2; ++i) {
        string path = AStar(decided, pos[i * n + i + 1], Point(i, i));
        for(char k : path) {
        ...
        while(j < n - 2 || k < n - 2) {
            if(k == n - 2) {
                path = AStar(decided, pos[i * n + j + 1], Point(i, j));
                for(char c : path) {
                    ans += move(c, pos[i * n + j + 1], pos[0]);
                }
                decided[i][j] = 1;
                ++j;
            } else if(j == n - 2) {
                path = AStar(decided, pos[k * n + i + 1], Point(k, i));
                for(char c : path) {
                    ans += move(c, pos[k * n + i + 1], pos[0]);
                }
                decided[k][i] = 1;
                ++k;
            } else {
                string path1 = AStar(decided, pos[i * n + j + 1], Point(i, j));
                string path2 = AStar(decided, pos[k * n + i + 1], Point(k, i));
