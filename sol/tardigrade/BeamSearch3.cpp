...
class Node {
  ...
  public:
    ...
    Node(int ul, int ll, int wi, const vector<int> &G)
        : under_limit(ul), left_limit(ll), width(wi), board(G), ban(n * n) {
        this->turn = 0;
        this->score = 0;
        this->correct = 0;
        this->hash = 0;
...
class State {
  ...
  public:
    ...
    void doAction(int start_x, int start_y, int width) {
        ...
        Node ans = beamSearchActionSimpleWithTurn(start, 30, 100);
...
bool operator>(const State &state_1, const State &state_2) {
    if(state_1.score != state_2.score) {
        return state_1.score > state_2.score;
    }
...
State beamSearchActionSimple(const State &state, const int beam_width,
    ...
    // 初期状態を追加
    now_beam.emplace(std::make_shared<State>(state));
    ...
    for(int t = 0;; t++) {
        ...
        for(int i = 0; i < beam_width; i++) {
            ...
            for(int j = 0; j < n; j += 3) {
                for(int k = 0; k < n; k += 3) {
                    ...
                    next_state.doAction(min(j, n - width), min(k, n - width),
                                        width);
                    ...
                    next_beam.emplace(std::make_shared<State>(next_state));
        ...
        if(best_state.isDone() || now_beam.empty() || t == 20) {
            break;
        }
    }
...
int main() {
    ...
    State ans = beamSearchActionSimple(start, 10, width);
    cerr << "ok" << endl;
    for(int i = 0; i < n; ++i) {
        for(int j = 0; j < n; ++j) {
